...
from models import LitAutoEncoder, QNet
print('=============================================================')
message="This is a scrpit to create a quantized model using Torch-qat(Quantizaiton Aware Training) \n out of a model pretrained with PyTorch lightning"
print(message)
print('=============================================================')
...
# End of setting
    ...
    qmodel_fp32_fused = torch.quantization.fuse_modules(qmodel.model, [['conv1', 'relu1'],['conv2', 'relu2']])
    qmodel_fp32_prepared = torch.quantization.prepare_qat(qmodel)
    qmodel_fp32_prepared.eval()
    qmodel_int8 = torch.quantization.convert(qmodel_fp32_prepared)
    #Saving converted model and prepared model
...
#Try to infer using a quantized model
    inference_with_torch=False
    #Change the boolean value above to 'True' in order to infer with PyTorch
    if inference_with_torch:
        qmodel_int8.eval()
        test_loss = 0
        correct = 0
    ...
    #Inference with PyTorchLightning failed because of the same Error when I had tried with PyTorch inference
        trainer = pl.Trainer(accelerator="auto")
        trainer.test(qmodel_int8)
    if args.named_modules or args.named_parameters:
        print("=====================")
